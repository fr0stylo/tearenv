...
func New(config Config) (*Server, error) {
	...
	if config.Gateway == nil {
		config.Gateway = NewLifecycleGateway(config.Credentials, nil, config.Logger)
	}
	...
	sshServer := &gliderssh.Server{
		...
		PasswordHandler: func(ctx gliderssh.Context, password string) bool {
			...
			if enrollmentIdentity, enrollment := protocol.EnrollmentIdentity(identity); enrollment {
				authenticated := config.Credentials.AuthenticateInvite(enrollmentIdentity, password)
				if authenticated {
			...
			authenticated := config.Credentials.Authenticate(identity, password)
			if authenticated {
				...
				config.Logger.Warn("client authentication rejected", "identity", identity, "remote", ctx.RemoteAddr())
			}
			return authenticated
		},
		...
		RequestHandlers: map[string]gliderssh.RequestHandler{
			protocol.EnrollRequestType:   enrollmentHandler(config.Credentials, config.Logger),
