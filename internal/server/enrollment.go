...
func enrollmentHandler(enrollment authorization.Enrollment, logger *slog.Logger) gliderssh.RequestHandler {
	return func(ctx gliderssh.Context, _ *gliderssh.Server, _ *ssh.Request) (bool, []byte) {
		...
		token, err := enrollment.Enroll(attempt.Identity, attempt.Invite)
		if err != nil {
