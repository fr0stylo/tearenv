...
## Use OIDC instead of a registration token

OIDC is an alternative mode, not an additional authentication layer. Use the
optional bundled Dex dependency, or connect an existing Dex, Keycloak, or
standards-compatible provider. Create a public native application and enable
authorization code with PKCE and loopback redirect URIs. ID-token exchange is
the default. See [the OIDC provider guide](oidc-providers.md) for runnable Dex,
GitHub, Google, and Keycloak configurations.

Create the SSH user CA Secret:
...
Then replace the token values with:
...
OIDC login needs permanent access to the registration API, including discovery,
registration, and token exchange. Enable the TLS Ingress and use its HTTPS URL.
The CA Secret isn't generated by Helm and isn't stored on the state PVC.

To deploy Dex with the same release, set
`registration.oidc.provider=bundledDex`, set `dex.enabled=true`, and configure
`dex.config.issuer`, a TLS Ingress, and at least one connector. The chart derives
the tearenv issuer from Dex and validates the public client. Start from
`deploy/helm/tearenv/examples/dex-github-values.yaml`.
...
Also back up the active authentication Secret: the registration token in token
mode or the SSH user CA in OIDC mode. Protect backups as authentication and
network-policy data. Losing the OIDC CA doesn't lose registrations, but it
requires a deliberate CA rotation and invalidates certificates from the old CA.
